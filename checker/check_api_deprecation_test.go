...
func TestApiDeprecated_DetectsDeprecatedOperations(t *testing.T) {
	...
	require.Equal(t, checker.EndpointDeprecatedWithSunsetId, e0.Id)
...
func TestApiDeprecated_MessageIncludesSunsetAndStability(t *testing.T) {
	...
	var deprecationChange checker.Change
	for _, e := range errs {
		if e.GetId() == checker.EndpointDeprecatedWithSunsetId {
			deprecationChange = e
		}
	}
	require.NotNil(t, deprecationChange, "expected endpoint-deprecated-with-sunset change")
