	...
	walkModifiedResponseSchemas(diffReport, operationsSources, config, func(info mediaTypeInfo) {
		...
		if responseTypeFormatBreaking(typeDiff, formatDiff, info.mediaType, schemaDiff) {
			...
			result = append(result, info.newChange(
				...
				[]any{getBaseTypeFormat(schemaDiff), getRevisionTypeFormat(schemaDiff), info.responseStatus},
				"",
		...
		info.walkProperties(func(p propertyInfo) {
			...
			if responseTypeFormatBreaking(propTypeDiff, propFormatDiff, info.mediaType, propSchemaDiff) {
				...
				result = append(result, p.newChange(
					...
					[]any{propertyFullName(p.propertyPath, p.propertyName), getBaseTypeFormat(propSchemaDiff), getRevisionTypeFormat(propSchemaDiff), info.responseStatus},
					"",
