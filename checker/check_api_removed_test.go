...
func TestBreaking_DeletedPath(t *testing.T) {
	...
	require.Equal(t, checker.NewSource("../data/openapi-test1.yaml", 175, 5).WithEnd(232, 29), errs[0].GetBaseSource())
...
func TestBreaking_RemoveBeforeSunset(t *testing.T) {
	...
	require.Equal(t, checker.NewSource("../data/deprecation/deprecated-future.yaml", 11, 5).WithEnd(16, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
...
func TestBreaking_DeprecationNoSunset(t *testing.T) {
	...
	require.Equal(t, checker.NewSource("../data/deprecation/deprecated-no-sunset.yaml", 11, 5).WithEnd(15, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
...
func TestBreaking_RemovedPathForAlphaBreaking(t *testing.T) {
	...
	requireSingleChange(t, errs, checker.APIPathRemovedWithoutDeprecationId)
	require.Equal(t, "api path removed without deprecation", errs[0].GetUncolorizedText(checker.NewDefaultLocalizer()))
	require.Equal(t, checker.NewSource("../data/deprecation/base-alpha-stability.yaml", 7, 5).WithEnd(10, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
...
func TestBreaking_RemovedPathForDraftBreaking(t *testing.T) {
	...
	requireSingleChange(t, errs, checker.APIPathRemovedWithoutDeprecationId)
	require.Equal(t, "api path removed without deprecation", errs[0].GetUncolorizedText(checker.NewDefaultLocalizer()))
	require.Equal(t, checker.NewSource("../data/deprecation/base-draft-stability.yaml", 7, 5).WithEnd(10, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
...
func TestBreaking_DeprecationPathMixed(t *testing.T) {
	...
	require.Equal(t, checker.NewSource("../data/deprecation/deprecated-path-mixed.yaml", 18, 5).WithEnd(23, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
...
func TestBreaking_RemoveEndpointWithInvalidSunset(t *testing.T) {
	...
	require.Equal(t, checker.NewSource("../data/deprecation/deprecated-invalid.yaml", 11, 5).WithEnd(16, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
...
func TestBreaking_DeprecationPathMixed_RFC3339_Sunset(t *testing.T) {
	...
	requireSingleChange(t, errs, checker.APIPathRemovedBeforeSunsetId)
	require.Equal(t, "api path removed before the sunset date `9999-08-10`", errs[0].GetUncolorizedText(checker.NewDefaultLocalizer()))
	require.Equal(t, checker.NewSource("../data/deprecation/deprecated-path-mixed-rfc3339-sunset.yaml", 19, 5).WithEnd(24, 26), errs[0].GetBaseSource())
	require.Empty(t, errs[0].GetRevisionSource())
}
