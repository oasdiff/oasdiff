...
func checkPropertyStabilityChange(
	...
	// Determine what to report based on whether --stability-level flag is set
	isDecrease := revisionLevel < baseLevel

	if config.StabilityLevel == StabilityLevelNone {
		// Default (no flag): only report stable→beta decrease
		if !(baseLabel == STABILITY_STABLE && revisionLabel == STABILITY_BETA) {
			return
		}
	} else {
		// With flag: report if either base or revision meets the configured threshold
