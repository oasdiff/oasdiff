...
// into, so a field added upstream shows up as new cells without a manual
// update here.
//
// Actions are syntactic (add, remove, set, unset, change, increase,
// decrease), not verdicts: whether an edit at a cell is breaking is the
// checker's judgment, recorded by mapping rules to cells. Polarity is the
...
// current path, so a cell like paths.*.*.requestBody.content.*.schema.maxLength
