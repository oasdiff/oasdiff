...
A change inside an `allOf` is the common case: without [`--flatten-allof`](DIFF.md) the branches are compared one at a time, so another branch may still guarantee what one branch dropped. Such a change is reported at `WARN` even where its check declares `ERR`, and the comment points at the flag.
