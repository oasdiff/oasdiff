...
func addCommonDiffFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().Bool("match-inline-refs", true, "match validation-equivalent inline/$ref subschemas under anyOf/oneOf as the same branch (treats codegen refactors as no-change); set to false to restore the pre-1.17 behaviour where the inline-to-$ref form change is reported as one added and one removed branch")
