...
func getOneOfWrappingDiff(config *Config, state *state, base, revision *openapi3.Schema) *OneOfWrappingDiff {
	...
	return &OneOfWrappingDiff{
		...
		OriginalPreserved: anyAlternativeEquivalent(config, state, base, revision.OneOf),
	}
}
...
func anyAlternativeEquivalent(config *Config, state *state, base *openapi3.Schema, alts openapi3.SchemaRefs) bool {
	baseRef := &openapi3.SchemaRef{Value: base}
	for _, alt := range alts {
		if schemaRefsValidationEquivalentWithin(config, state, baseRef, alt) {
			return true
		}
	}
	return false
