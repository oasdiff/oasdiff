...
func TestGetNullableWrappingDiff(t *testing.T) {
	...
	require.False(t, getNullableWrappingDiff(cfg, newState(), str, wrap(null, str)).Empty(), "the pure wrap is recognized")
	require.False(t, getNullableWrappingDiff(cfg, newState(), str, wrap(str, null)).Empty(), "branch order does not matter")

	nullableStr := &openapi3.Schema{Type: &openapi3.Types{"string", "null"}}
	require.True(t, getNullableWrappingDiff(cfg, newState(), nullableStr, wrap(null, nullableStr)).Empty(),
	...
	require.True(t, getNullableWrappingDiff(cfg, newState(), str, wrap(null, other)).Empty(),
		"a non-equivalent branch is not recognized")

	require.True(t, getNullableWrappingDiff(cfg, newState(), str, wrap(null, str, str)).Empty(),
		"only a two-branch wrap is recognized")

	nullWithEnum := &openapi3.Schema{Type: &openapi3.Types{"null"}, Enum: []any{"x"}}
	require.True(t, getNullableWrappingDiff(cfg, newState(), str, wrap(nullWithEnum, str)).Empty(),
	...
	require.True(t, getNullableWrappingDiff(cfg, newState(), str, constrained).Empty(),
		"a wrapper with its own constraints is not bare")

	annotated := wrap(null, str)
	annotated.Description = "now nullable"
	require.False(t, getNullableWrappingDiff(cfg, newState(), str, annotated).Empty(),
	...
	require.False(t, getNullableWrappingDiff(cfg, newState(), str, wrap(nullDescribed, str)).Empty(),
