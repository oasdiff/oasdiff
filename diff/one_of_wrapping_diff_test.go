...
func TestGetOneOfWrappingDiff_OriginalPreserved(t *testing.T) {
	...
	require.True(t, getOneOfWrappingDiff(cfg, newState(), base, wrap(same, other)).OriginalPreserved,
		"an alternative with the base's validation contract preserves the original")
	require.True(t, getOneOfWrappingDiff(cfg, newState(), base, wrap(other, same)).OriginalPreserved,
		"branch order does not matter")

	relaxed := obj(nil, map[string]*openapi3.Schema{"foo": str(), "bar": str()})
	require.False(t, getOneOfWrappingDiff(cfg, newState(), base, wrap(relaxed, other)).OriginalPreserved,
	...
	require.False(t, getOneOfWrappingDiff(cfg, newState(), base, wrap(narrowed, other)).OriginalPreserved,
	...
	require.True(t, getOneOfWrappingDiff(cfg, newState(), base, wrap(annotated, other)).OriginalPreserved,
